...
# Enum.map/Stream.map |> sum => sum_by (Elixir 1.18+)
a |> Enum.map(mapper) |> Enum.sum() |> ...
# Styled:
a |> Enum.sum_by(mapper) |> ...

# Enum.map/Stream.map |> product => product_by (Elixir 1.18+)
a |> Enum.map(mapper) |> Enum.product() |> ...
# Styled:
a |> Enum.product_by(mapper) |> ...

# Map.values |> sum_by/product_by => aggregate over map entries (Elixir 1.18+)
mapping |> Map.values() |> Enum.sum_by(mapper) |> ...
# Styled:
mapping |> Enum.sum_by(fn {_, value} -> mapper.(value) end) |> ...

mapping |> Map.values() |> Enum.product_by(mapper) |> ...
# Styled:
mapping |> Enum.product_by(fn {_, value} -> mapper.(value) end) |> ...

# The same rewrite composes with map |> sum/product.
mapping |> Map.values() |> Enum.map(mapper) |> Enum.product() |> ...
# Styled:
mapping |> Enum.product_by(fn {_, value} -> mapper.(value) end) |> ...
