...
- Rewrite `Enum.map/2` or `Stream.map/2` followed by `Enum.sum/1` or `Enum.product/1` to `Enum.sum_by/2` or `Enum.product_by/2` on Elixir 1.18+; fuse a preceding `Map.values/1` into the aggregate function.
